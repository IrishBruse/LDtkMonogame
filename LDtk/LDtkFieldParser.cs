...
namespace LDtk
{
    ...
    internal static class LDtkFieldParser
    {
        ...
        internal static void Parse<T>(T entity, LDtkLevel level) where T : new()
        {
            for (int i = 0; i < level.FieldInstances.Length; i++)
            {
                var fieldInstance = level.FieldInstances[i];
                    ...
                    case Field.PointType:
                        if (fieldInstance._Value != null)
                        {
                            ...
                            else if (field.PropertyType == typeof(Point))
                            {
                                ...
                                // We need to vconvert the grid space to world pixel space
                                level.
